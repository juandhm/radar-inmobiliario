...
Script para procesar JSON de Ciencuadras y generar archivos estandarizados para análisis de mercado.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('procesador_ciencuadras.log'),
...
class ProcesadorCiencuadras:
    """Clase principal para procesar datos inmobiliarios de Ciencuadras."""

    def __init__(self, nombre_plataforma: str = "ciencuadras"):
        self.nombre_plataforma = nombre_plataforma

    def cargar_json_original(self, ruta_archivo: str) -> Dict:
        """Cargar el archivo JSON original de Ciencuadras."""
        try:
    ...
    def extraer_lista_inmuebles(self, datos: Dict) -> List[Dict]:
        """Extraer solo la lista de inmuebles del JSON de Ciencuadras."""
        try:
            data = datos.get('data', {})
            inmuebles: List[Dict] = []
            for clave in ['highlights', 'results']:
                lista = data.get(clave, [])
                if isinstance(lista, list):
                    inmuebles.extend([x for x in lista if isinstance(x, dict)])
            return inmuebles
        except Exception as e:
    ...
    def mapear_campos_inmueble(self, inmueble: Dict) -> Dict:
        ...
        inmueble_estandarizado['url_anuncio'] = f"https://www.ciencuadras.com{inmueble.get('url', '')}"

        # Ubicación
        inmueble_estandarizado['departamento'] = inmueble.get('department', '')
        inmueble_estandarizado['ciudad'] = inmueble.get('city', '')
        # Ciencuadras trae neighbourhood en múltiples variantes; priorizar 'neighborhood'
        inmueble_estandarizado['barrios'] = (
            inmueble.get('neighborhood') or inmueble.get('neighborhood_integrator') or inmueble.get('locality') or ''
        )
        inmueble_estandarizado['direccion'] = inmueble.get('address', '')

        # Coordenadas
        coords = inmueble.get('coordinates', {})
        if isinstance(coords, dict):
            inmueble_estandarizado['latitud'] = coords.get('latitude', '')
            inmueble_estandarizado['longitud'] = coords.get('longitude', '')

        # Características
        inmueble_estandarizado['tipo_propiedad'] = inmueble.get('realEstateType', '')
        inmueble_estandarizado['area_total_m2'] = inmueble.get('area', '')
        inmueble_estandarizado['num_habitaciones'] = inmueble.get('rooms', '')
        inmueble_estandarizado['num_banos'] = inmueble.get('baths', '')
        inmueble_estandarizado['num_parqueaderos'] = inmueble.get('garages', '')
        inmueble_estandarizado['estrato'] = inmueble.get('stratum', '')

        # Estado / Antigüedad
        inmueble_estandarizado['estado'] = None
        inmueble_estandarizado['antiguedad_construccion'] = None

        # Precios
        inmueble_estandarizado['precio_venta'] = inmueble.get('salePrice', '')
        # adminValue es entero total, no diferencia precio base vs admin; guardarlo como está
        inmueble_estandarizado['precio_administracion'] = inmueble.get('adminValue', '')

        # Fechas
        inmueble_estandarizado['fecha_publicacion'] = inmueble.get('createdAt', '')
        inmueble_estandarizado['fecha_actualizacion'] = inmueble.get('createdAt', '')
        inmueble_estandarizado['fecha_ingreso'] = inmueble.get('createdAt', '')

        # Contacto
        inmueble_estandarizado['nombre_contacto'] = inmueble.get('userName', '')
...
def main():
    ...
    try:
        ...
        procesador = ProcesadorCiencuadras()

        # Ejecutar procesamiento completo
        procesador.procesar_completo('ciencuadras.json')
